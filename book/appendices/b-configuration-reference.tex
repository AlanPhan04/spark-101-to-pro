...
\texttt{write.delete.mode} \newline \texttt{write.update.mode} \newline
  \texttt{write.merge.mode} & \texttt{copy-on-write} & Sets copy-on-write
  vs.\ merge-on-read independently per operation type. Iceberg has changed
  this default across releases, so confirm with
  \texttt{SHOW TBLPROPERTIES} rather than assuming
