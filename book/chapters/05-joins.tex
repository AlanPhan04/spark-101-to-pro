...
--- \texttt{ON a.price > b.min\_price}, for instance --- needs neither, and
...
SELECT /*+ BROADCAST(r) */ t.route_id, r.route_name
FROM trips t JOIN dim_route r ON t.route_id = r.route_id;
\end{lstlisting}
...
ANALYZE TABLE db.dim_route COMPUTE STATISTICS FOR ALL COLUMNS;
\end{lstlisting}
...
join --- which is exactly why the specimen query's small \texttt{dim\_route}
...
Every strategy above assumes the two sides arrive unaligned and have to be
shuffled into agreement. If both tables are already \emph{bucketed} on the join
...
\item Run the specimen query, then run \texttt{ANALYZE TABLE dim\_route
